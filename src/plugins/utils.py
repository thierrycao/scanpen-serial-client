...
def write_bin_list_to_file(src_list, output='output.bin', signed=False):
    ...
    try:
        with open(output, 'wb') as wf:
            if isinstance(src_list, list):
                for i in src_list:
                    # wf.write(bytes(i))
                    wf.write(i.to_bytes(1, byteorder='little', signed=signed))
            else:
...
def numpy_to_bytes(image_np):    
    ...
    return image_bytes
def bin_list_to_bmp_bytes(bin_list, width, height):
    from PIL import Image
    import numpy as np
    img_gray_8 = Image.new('L', (width, height), 128)
    img = np.array(img_gray_8)
